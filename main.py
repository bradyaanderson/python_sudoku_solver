# prints out sudoku grid in a readable format
def print_grid():
	for row in grid:
...
# checks if board has been solved or not
def check_complete():
	if len([i for i in range(9) if 0 in grid[i]]) != 0:
		return False
	return True

# builds column given x value
def make_col(x):
	return [grid[i][x] for i in range(9)]

# builds box given x and y value
def make_box(x,y):
	box_x = x // 3
	box_y = y // 3
...
def find_possible(x,y):
	row_poss = find_row_possible(y)
	col_poss = find_col_possible(x,y)
	box_poss = find_box_possible(x,y)

	# if only one value is possible, returns value
...
# finds possible solutions for row given y value
def find_row_possible(y):
	return set([i for i in range(1,10) if i not in grid[y]])

# finds possible solutions for column given x and y value
def find_col_possible(x,y):
	col = make_col(x)
	return set([i for i in range(1,10) if i not in col])

# finds possible solutions for box given x and y value
def find_box_possible(x,y):
	box = make_box(x,y)
